...
export const createProject = async (req, res) => {
  try {
    const { title, description, type, isFree, isFeatured, price, githubUrl, liveUrl } = req.body;
    ...
    if (!freeStatus && (!price || parseFloat(price) <= 0)) {
      return res.status(400).json({ success: false, message: 'Price is required for paid projects' });
    }
